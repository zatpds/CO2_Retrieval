...
spec_data_path = '/net/fusi/raid06/shc/Bulk_Data/OCO_Forward/Riyadh_new/aod_plume_0_rd.npy' # Just need to care about AOD up to 1
...
iidx = L1bDict['Indices'][0]

try:
    sol_noc_rec = np.load('./Synthetic_Retrieval/Apr11_aod_noc.npy')
    sol_ret_rec = np.load('./Synthetic_Retrieval/Apr11_aod_ret.npy')
    noc_flags = np.load('./Synthetic_Retrieval/Apr11_aod_noc_flags.npy')
    ret_flags = np.load('./Synthetic_Retrieval/Apr11_aod_ret_flags.npy')
...
print(len(noc_flags)) # 0. Why?

for i in [len(noc_flags)]: # range(spec_data.shape[0])
    tmp = spec_data[i,:,:]
    OCO2_Solution_Toolkit.set_y_l1b_file(tmp,iidx,l1bFile)
    ...
    x_a = l2_obj.get_x()
    print(np.mean(x_a[0:20]))
    S_a = l2_obj.get_Sa()
    Kupdate = l2_obj.Kupdate
    flag = 1
    x = []
    try:
        l2_obj.solve(x_i=x_a,x_a=x_a,cov_a=S_a)
        x = l2_obj.get_x()
    except Exception as e:
        ...
        print(e)
        flag = 0
    noc_flags = np.append(noc_flags,flag)
    if len(noc_flags)==1:
        sol_noc_rec = np.array(x)
    else:
        sol_noc_rec = np.vstack((sol_noc_rec,x))
    ...
    x_a[35] = -9999
    l2_obj.set_x(x_a)
    l2_obj = OCO2_Solution_Toolkit.constrain_aod_err(l2_obj, aod, aod_sig)
    l2_obj = OCO2_Solution_Toolkit.constrain_alh_err(l2_obj, 1-alh, alh_sig)
    Kupdate = l2_obj.Kupdate
    flag = 1
    try:
        x_a = l2_obj.get_x()
        ...
        print(e)
        flag = 0
    ret_flags = np.append(ret_flags,flag)
    if len(ret_flags)==1:
        sol_ret_rec = np.array(x)
    else:
        sol_ret_rec = np.vstack((sol_ret_rec,x))

print('Calculations all done. Now saving!!!')
np.save('./Synthetic_Retrieval/Apr11_aod_noc.npy',sol_noc_rec)
np.save('./Synthetic_Retrieval/Apr11_aod_ret.npy',sol_ret_rec)
np.save('./Synthetic_Retrieval/Apr11_aod_noc_flags.npy',noc_flags)
np.save('./Synthetic_Retrieval/Apr11_aod_ret_flags.npy',ret_flags)